...
public struct PDFDocument {
    ...
    ///
    /// - returns: A newly initialized `PDFDocument`
    fileprivate init?(fileData: Data, fileURL: URL?, fileName: String, password: String?) {
    ...
    func allPageImages(_ callback: ([UIImage]) -> Void) {
        var images = [UIImage]()
    ...
    ///
    /// - returns: Image representation of the document page
    fileprivate func imageFromPDFPage(at pageNumber: Int, callback: (UIImage?) -> Void) {
